...
metrics_sklearn = ['cosine similarity', 'cosine distance', 'euclidean distance', 'manhattan distance', 'cityblock distance', 'l1 distance', 'l2 distance']
...
class SemanticSpace(object):
    ...
    def most_similar(self, l1, l2=None, n=10, metric='cosine', type='normal'):
        ...
        if metric == 'cosine_sim':
            for word, neighbours in sims.iterrows():
                neigh_indexes = neighbours.argsort().values[::-1][:n]
    ...
    def postprocess_vectors(self, d=None):
        ...
        """
        self.vectors = space_utils.postprocess(self.vectors, d=d)
